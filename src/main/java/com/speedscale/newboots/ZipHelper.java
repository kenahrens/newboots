 ...
 * This class provides methods to download and analyze ZIP files.
 */
public final class ZipHelper {

  /** Logger for this class. */
  private static final Logger LOGGER =
      LoggerFactory.getLogger(ZipHelper.class);
  ...
  static {
    ZIP_URLS.put("speedscale",
      "https://github.com/speedscale/speedscale/archive/refs/heads/main.zip");
    ZIP_URLS.put("jquery",
      "https://github.com/jquery/jquery/archive/refs/heads/main.zip");
    ZIP_URLS.put("bootstrap",
    ...
    try {
      ...
      if (filename != null && !filename.trim().isEmpty()) {
        ...
        if (ZIP_URLS.containsKey(filename.toLowerCase())) {
          ...
          LOGGER.info("[{}] Processing predefined ZIP file: {}",
            requestId, filename);
        } else {
          // Fallback to learningcontainer pattern for backward compatibility
          zipUrl =
            "https://www.learningcontainer.com/wp-content/uploads/2020/05/"
            + filename;
          LOGGER.info("[{}] Processing custom ZIP file (may not exist): {}",
            requestId, filename);
        }
      } else {
        // Use default speedscale zip file
        zipUrl = ZIP_URLS.get("speedscale");
        LOGGER.info("[{}] Processing default ZIP file (speedscale)",
          requestId);
      }
      ...
      LOGGER.info("[{}] REQ zip download: {}",
          requestId, httpget.getRequestUri());

      String result = HTTP_CLIENT.execute(httpget, response -> {
          HttpEntity entity = response.getEntity();
          LOGGER.info("[{}] RES zip download: {}",
          ...
          if (response.getCode() == HTTP_OK) {
              ...
              String errorMsg = String.format(
                  "Failed to download zip file from URL: %s", zipUrl);
              LOGGER.error("[{}] ZIP download failed - Status: {}, URL: {}",
                  requestId, response.getCode(), zipUrl);
      ...
      body = result;
        ...
        LOGGER.error("[{}] Unable to download and process zip file",
            requestId, e);
        throw e;
    }
   ...
   */
  private static List<ZipFileInfo> getZipIndex(final byte[] zipBytes)
      throws IOException {
   ...
   */
  private static String convertToJson(final List<ZipFileInfo> zipIndex)
      throws Exception {
