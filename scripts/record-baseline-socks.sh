#!/usr/bin/env bash
# Record baseline traffic (NASA, SpaceX) using SOCKS proxy

set -euo pipefail

echo "=== Recording Baseline Traffic with SOCKS Proxy ==="
echo "Captures: NASA API, SpaceX API, and other external calls"

# Configuration
TS=$(date +%Y-%m-%d_%H-%M-%S)
OUT_DIR="proxymock/recorded-${TS}-baseline-socks"
SOCKS_PORT=4140
APP_PORT=8080
...
proxymock record --out "$OUT_DIR" --app-port $APP_PORT > proxymock-record.log 2>&1 &
PM_PID=$!
sleep 3
...
# Start application with SOCKS proxy
...
echo "🔧 Starting application with SOCKS proxy..."
...
curl -s "http://localhost:$APP_PORT/number-to-words?number=123" || echo "SOAP API called"
